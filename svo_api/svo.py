...
def get_filters(facility, filters_sets=None, verbose=False):
    ...
    if filters_sets is None:
        ...
        for filter_set in filters_sets:
            assert filter_set in valid_filters_sets, f"Not a valid filter set ({filter_set}): {valid_filters_sets}"
    filters = []
    
    for filter_set in filters_sets:
        # get the facility + filters set's SVO URL
        filters_url = search_url + f'mode=browse&gname={facility}&gname2={filter_set}&asttype='
